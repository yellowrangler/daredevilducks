...
def processPlay (gameSeason)
	...
	gameSeason.each do |game|
		...
		tempgamedate = game["Date"]
		tempgamedateArray = tempgamedate.split("-")
		gamedate = tempgamedateArray[1] + " " + tempgamedateArray[0]	
	...
	# 
	if File.exist?('nfl2015-insert-play.sql')
		File.delete('nfl2015-insert-play.sql')
	end

	File.write('nfl2015-insert-play.sql', sqlStr)
end
...
def processBye(gameSeason)
	...
	# write string to file
	# 
	if File.exist?('nfl2015-insert-bye.sql')
		File.delete('nfl2015-insert-bye.sql')
	end

	File.write('nfl2015-insert-bye.sql', sqlStr)
end
