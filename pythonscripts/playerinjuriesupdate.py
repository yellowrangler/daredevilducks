...
# 
#  get injury data to show
#  
def show_injury_data(injury_season, injury_week):
  try:
    # Connect to the MySQL database Create a buffered cursor
    cnx = mysql.connector.connect(**dbparms)
    cursor = cnx.cursor(buffered=True)
...
answer = input(f"Would you like the Injury conversion data frame to be applied to database ?: (y/n) ")
...
answer = input(f"Would you like to display data applied to database ?: (y/n) ")
if (answer[0] == 'y' or answer[0] == 'Y'):
  pass
else:
  print("Injury conversion finished!\n")
  quit()

# 
# print injury stats from data base
# 
show_results = show_injury_data(injury_season, injury_week)
for row in show_results:
  print(row)

# print("\nNow show pretty print\n")
# pp(show_results, indent=4)
